...
| `create-aem-page` | Create a new page under a parent path. For Universal Editor / Edge Delivery (AEM_EDGE) pages you MUST use the modern API: provide a valid `templateId` (the reliable, primary way — list `GET /adobe/pages/templates` filtered by siteId and pick any returned template) or `sourcePageId` (clone an existing page — a secondary option; NOT reliable when the source page's parent is a folder, common for Edge-Delivery content, where it returns HTTP 500). Prefer `templateId` unless an explicit clone of an existing page is wanted. NOTE: a page's own `templateId` (from get-aem-page-metadata) is NOT necessarily a usable create template — it can return 400 'template not found'. Always get `templateId` from `GET /adobe/pages/templates`, not from an existing page's metadata. Passing a legacy JCR `template` path instead falls back to the classic WCM command and produces a CLASSIC page that will NOT open in the Universal Editor and cannot be edited via the modern page-content tools. WORKFLOW (modern): 1) identify parentPath (e.g. '/content/mysite/en'); 2) provide templateId (preferred) or sourcePageId; 3) provide title and name (URL segment). Note: use copy-aem-page to copy an existing page instead. ⚠️ This is a PERSISTING WRITE with NO dry-run/validate/preview mode. Call it EXACTLY ONCE per intended page — there is nothing to test or preview against, and calling it again to 'verify' the first call creates a second real page (auto-suffixed by AEM). |
...
| `get-aem-cf-delivery-html` | Fetch the rendered HTML for a materialized Content Fragment delivery URL (sets Accept: text/html to avoid the delivery CDN's 406). Pass the authorUrl you published through and the deliveryUrl returned by publish-aem-cf-html-fragment; the deliveryUrl must be the preview-/publish-/delivery- tier of that same environment. Supports AEM Cloud Service environments with autogenerated '<tier>-p<N>-e<N>' delivery FQDNs; a custom-mapped delivery domain isn't supported here — fetch such a render directly. |
...
| `publish-aem-cf-html-fragment` | Materialize a Content Fragment variation's rendered HTML on a delivery tier and return the authoritative deliveryUrl. Publish the fragment CONTENT to the target tier FIRST (manage-fragments operation=publish with tier), else this returns 409. Use tier='preview' for a pre-approval review render (non-live), tier='publish' for the live render. Fetch the rendered HTML from the returned deliveryUrl via get-aem-cf-delivery-html. |
